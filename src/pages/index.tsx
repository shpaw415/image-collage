...
type ProjectType = {
	...
	uploadedImages: { id: string; blob: Blob }[];
	collageImages: ({ id: string; blob: Blob } | null)[];
};
...
export default function HomePage() {
	...
	const MainKeyUp = useCallback(() => setActiveSlot(null), []);
	const MainDragOver = useCallback<DragEventHandler<HTMLDivElement>>((e) => {
		e.preventDefault();
		e.stopPropagation();
	}, []);
	const MainOnDrop = useCallback<DragEventHandler<HTMLDivElement>>(
		(e) => {
			e.preventDefault();
			e.stopPropagation();
						...
						>
							{Array.from({ length: getSlotCount() }).map((_, index) => {
								...
								return (
									<figure
										key={`empty-${index}`}
										className={`flex flex-col items-center justify-center h-full w-full min-h-37.5 min-w-37.5 border-2 border-dashed transition-colors cursor-pointer ${
											activeSlot === index
...
function SideBar() {
	...
	const handleLoadProject = async (id: string) => {
		try {
			const db = await getDb();
			const proj = (await db.get("projects", id)) as ProjectType | undefined;
			if (!proj) return;

			const restoreUploaded = await Promise.all(
				proj.uploadedImages.map(async (img) => ({
					id: img.id,
			...
			const restoreCollage = await Promise.all(
				proj.collageImages.map(async (img) => {
					if (!img) return null;
	...
	return (
		<div className="w-full md:w-80 shrink-0 bg-white p-4 md:p-6 shadow-md flex md:flex-col gap-4 md:gap-6 overflow-x-auto md:overflow-y-auto z-10 custom-scrollbar max-h-[30vh] md:max-h-none border-b md:border-b-0 md:border-r print:hidden">
			...
			{/* Projects */}
			<div className="flex flex-col gap-2 min-w-50 md:min-w-0 md:border-b md:pb-6">
			...
			<div className="flex flex-col gap-2 min-w-[150px] md:min-w-0 md:border-b md:pb-6 pl-4 border-l md:border-l-0 md:pl-0">
				...
				<div className="flex flex-col gap-1 md:gap-2">
					<label
						className="text-xs md:text-sm font-medium text-gray-700"
						htmlFor="grid-template-select"
					>
						Grid Template
					</label>
					<select
						id="grid-template-select"
						value={template}
						onChange={(e) => {
